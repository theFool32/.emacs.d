    ...
    (pcase type
      (`headline
       (cond ((and (fboundp 'toc-org-insert-toc)
             ...
             ;; ==========================================
             ;; [新增逻辑]: 如果当前 Headline 包含链接，优先执行跳转
             ;; ==========================================
             ((save-excursion
                (beginning-of-line)
                ;; 在当前行搜索任意 Org 链接
...
(defun my/smart-fold-toggle ()
  "Toggle fold based on the active minor mode."
  (interactive)
  (cond
   ;; 如果启用了 treesit-fold，优先使用它 (Python, JS, Rust 等)
   ((bound-and-true-p treesit-fold-mode)
    (treesit-fold-toggle))

   ;; 如果是 LaTeX 或 Elisp (通常使用 outline-minor-mode)
   ((bound-and-true-p outline-minor-mode)
    (outline-toggle-children))

   ;; 最后的保底：使用 Evil 自带的折叠 (vimish-fold 或 hideshow)
   (t
    (evil-toggle-fold))))
...
(use-package treesit-fold
  ...
  :config
  (setq treesit-fold-summary-format " <...%d lines...> ")
  ;; 绑定到 Evil 标准键位
  (with-eval-after-load 'evil
    (define-key evil-normal-state-map (kbd "za") 'treesit-fold-toggle)
    (define-key evil-normal-state-map (kbd "zc") 'treesit-fold-close)
    (define-key evil-normal-state-map (kbd "zo") 'treesit-fold-open)
    (define-key evil-normal-state-map (kbd "zM") 'treesit-fold-close-all)
...
(with-eval-after-load 'evil
  (evil-define-key 'normal LaTeX-mode-map
    (kbd "za") 'outline-toggle-children
    (kbd "zc") 'outline-hide-subtree
    (kbd "zo") 'outline-show-subtree
    (kbd "zM") 'outline-hide-body
    (kbd "zR") 'outline-show-all))
...
(use-package general
  ...
  (leader-def
    ...
    "te" '(popterm-window-toggle :wk "popterm bottom")
    "tf" '(+popterm-toggle-dwim :wk "popterm float")
